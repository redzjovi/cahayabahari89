		...
		<form onsubmit={save} class="mt-6 grid gap-4 rounded-card border border-line bg-surface p-6 shadow-card">
			...
			<fieldset class="grid gap-1.5 text-sm font-semibold">
				...
				<div class="flex flex-wrap gap-3 font-normal">
					{#each perms as p}
						<label class="flex cursor-pointer items-center gap-1.5 text-sm"><input type="checkbox" checked={ePerms.includes(p.slug)} onchange={(e) => (ePerms = e.currentTarget.checked ? [...ePerms, p.slug] : ePerms.filter((x) => x !== p.slug))} class="accent-[var(--brand)]" />{p.slug}</label>
					{/each}
				</div>
			</fieldset>
