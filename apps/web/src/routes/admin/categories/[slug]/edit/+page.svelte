<script lang="ts">
	...
	const slugPreview = $derived(cName.trim() ? `${slugify(cName)}-${cId}` : '');
	...
	onMount(async () => {
		...
		try {
			const res = await adminSession.api('/api/admin/categories?page=1&limit=100');
			if (!res.ok) throw new Error('load');
			const rows = ((await res.json()) as { data: { id: number; slug: string; name: string; status: string }[] }).data;
...
<section class="content-wrap max-w-3xl py-10">
	...
	{:else}
		...
		<form onsubmit={save} class="mt-6 grid gap-4 rounded-card border border-line bg-surface p-6 shadow-card">
			...
			<Field label="Slug" hint={t().admin.slugAutoNote}>
				<span class="block w-full rounded-lg border border-line bg-band px-4 py-2.5 font-mono text-sm font-normal text-muted">{slugPreview || '—'}</span>
			</Field>
