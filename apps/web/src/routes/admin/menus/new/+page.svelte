<script lang="ts">
	...
	const hrefPreview = $derived.by(() => {
		const h = fHref.trim();
		if (!h) return '—';
		if (/^https?:\/\//i.test(h)) return h;
		try {
			return localize(h.startsWith('/') ? h : '/' + h, locale.current);
		} catch {
			return h;
		}
	});
	...
	async function save(e: SubmitEvent) {
		...
		saving = true;
		try {
			const res = await adminSession.api('/api/admin/menus', {
				method: 'POST',
				body: JSON.stringify({
					location: fLoc,
					labelId: fLabelId,
...
<svelte:head><title>{t().admin.newMenu} — Admin</title></svelte:head>
...
<section class="content-wrap max-w-3xl py-10">
	...
	<h1 class="mt-3 font-display text-3xl font-bold">{t().admin.newMenu}</h1>
	...
	{:else}
		...
		<form onsubmit={save} class="mt-6 grid gap-4 rounded-card border border-line bg-surface p-6 shadow-card">
			...
			<Field label={t().admin.linkHref} hint={t().admin.linkHint} required>
				<input bind:value={fHref} required maxlength="500" placeholder="/products — https://…" class="w-full rounded-lg border px-4 py-2.5 font-mono font-normal" />
			</Field>
			<Field label={t().admin.viewPage}><span class="block w-full truncate rounded-lg border border-line bg-band px-4 py-2.5 font-mono text-sm font-normal text-muted">{hrefPreview}</span></Field>
